...
const Header: React.FC = () => {
    ...
    const navLinkClass = (path: string) =>
        `px-3.5 py-2 rounded-full text-sm font-medium transition-all hover:scale-105 active:scale-95 ${
            isActive(path)
    ...
    const mobileNavLinkClass = (path: string) =>
        `block w-full px-4 py-3 rounded-xl text-sm font-medium transition-all active:scale-[0.98] ${
            isActive(path)
                ? 'text-cyan-700 dark:text-cyan-300 bg-cyan-50 dark:bg-cyan-950/80 border border-cyan-200 dark:border-cyan-800/80 font-semibold shadow-sm'
                : 'text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-900'
        }`;

    // Header shows only the EZJOB product mark (no Clarity/LENIX cluster) —
    // the full "Clarity x LENIX x EZJOB" cluster is reserved for the Footer.
    const defaultEzjobLogoSrc = isDark ? '/assets/ezjob-logo-dark.png' : '/assets/ezjob-logo-light.png';
    const ezjobLogoSrc = siteAssets.logoUrl || defaultEzjobLogoSrc;

    const Logo = () => (
        <Link
            to="/"
            className="flex items-center select-none py-2 px-4 rounded-2xl bg-gradient-to-r from-slate-50/80 via-white/90 to-slate-50/80 dark:from-slate-900/80 dark:via-slate-950/90 dark:to-slate-900/80 border border-slate-200/80 dark:border-slate-800/80 shadow-sm hover:shadow-md dark:shadow-cyan-500/5 hover:border-cyan-500/40 hover:scale-[1.03] active:scale-95 transition-all duration-300"
        >
            <img
                src={ezjobLogoSrc}
                alt="EZJOB"
    ...
    return (
        <header className="sticky top-0 z-50 bg-white/95 dark:bg-slate-950/95 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800 shadow-sm transition-colors duration-300">
            <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between min-h-[128px] py-2 sm:py-3">
                    ...
                    <div className="hidden md:flex items-center space-x-1 lg:space-x-4">
                        ...
                        {user?.role === UserRole.EMPLOYER || user?.role === UserRole.ADMIN ? (
                            ...
                            <Link
                                ...
                                className="px-3.5 py-2 rounded-full text-sm font-medium text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-900 transition-all hover:scale-105 active:scale-95"
                            >
                                {t('nav.forEmployers')}
                            </Link>
                        )}
                    ...
                    <div className="flex items-center gap-2 sm:gap-3">
                        ...
                        <button
                            ...
                            className="p-2.5 rounded-full border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900 text-slate-700 dark:text-cyan-400 hover:scale-105 active:scale-90 hover:border-cyan-500 transition-all cursor-pointer shadow-sm"
                        ...
                        {user ? (
                            <div className="hidden md:flex items-center gap-3">
                                ...
                                <button
                                    ...
                                    className="bg-slate-100 dark:bg-slate-900 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 font-mono text-xs uppercase px-3 py-2 rounded-full transition-all border border-slate-200 dark:border-slate-800 cursor-pointer hover:scale-105 active:scale-95"
                                >
                                    {t('nav.logout')}
                                </button>
                            </div>
                            ...
                            <div className="hidden md:flex items-center gap-2">
                                <Link
                                    to="/login"
                                    className="text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white font-mono text-xs uppercase px-4 py-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-900 transition-all hover:scale-105 active:scale-95"
                                >
                                    {t('nav.login')}
                                </Link>
                                <Link
                                    to="/register"
                                    className="bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white font-mono text-xs uppercase px-5 py-2.5 rounded-full font-semibold shadow-sm hover:shadow-md transition-all transform hover:-translate-y-0.5 active:scale-95"
                                >
                                    {t('nav.register')}
                                </Link>
                            </div>
                        ...
                        <button
                            ...
                            className="md:hidden p-2.5 rounded-full border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900 text-slate-700 dark:text-slate-200 hover:border-cyan-500 transition-all cursor-pointer shadow-sm active:scale-90"
                        >
                    ...
                    <div className="md:hidden pb-4 border-t border-slate-200 dark:border-slate-800 pt-3 space-y-1">
                        ...
                        {user?.role === UserRole.ADMIN && (
                            <Link
                                ...
                                className={`block w-full px-4 py-3 rounded-xl text-sm font-medium transition-all active:scale-[0.98] ${
                                    isActive('/admin/dashboard')
                                        ? 'text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-950/80 border border-amber-200 dark:border-amber-800/80 font-semibold shadow-sm'
                                        : 'text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-900'
                                }`}
                        ...
                        {user?.role === UserRole.EMPLOYER || user?.role === UserRole.ADMIN ? (
                            ...
                            <Link
                                ...
                                state={{ role: UserRole.EMPLOYER }}
                                onClick={closeMobileMenu}
                                className="block w-full px-4 py-3 rounded-xl text-sm font-medium text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-900 transition-all active:scale-[0.98]"
                            >
                                {t('nav.forEmployers')}
                            </Link>
                        )}
                        ...
                        <div className="pt-2 mt-2 border-t border-slate-200 dark:border-slate-800">
                            {user ? (
                                <div className="flex items-center justify-between gap-3 px-4 py-2">
                                    ...
                                    <button
                                        onClick={handleLogout}
                                        className="bg-slate-100 dark:bg-slate-900 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 font-mono text-xs uppercase px-3 py-2 rounded-full transition-all border border-slate-200 dark:border-slate-800 cursor-pointer active:scale-95"
                                    >
                                        {t('nav.logout')}
                                    </button>
                                </div>
                                ...
                                <div className="flex items-center gap-2 px-4">
                                    <Link
                                        to="/login"
                                        onClick={closeMobileMenu}
                                        className="flex-1 text-center text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white font-mono text-xs uppercase px-4 py-2.5 rounded-full border border-slate-200 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-900 transition-all active:scale-[0.98]"
                                    >
                                        {t('nav.login')}
                                    </Link>
                                    <Link
                                        to="/register"
                                        onClick={closeMobileMenu}
                                        className="flex-1 text-center bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white font-mono text-xs uppercase px-4 py-2.5 rounded-full font-semibold shadow-sm hover:shadow-md transition-all active:scale-[0.98]"
                                    >
                                        {t('nav.register')}
                                    </Link>
                                </div>
